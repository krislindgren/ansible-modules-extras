...
class HAProxy(object):
    ...
    def get_state_for(self, pxname, svname):
        ...
        state = []
        for d in r:
            if (pxname is None or d['pxname'] == pxname) and d['svname'] == svname:
                state.append({ 'status': d['status'], 'weight': d['weight'], 'scur': d['scur'] })
        return state
    ...
    def wait_until_status(self, pxname, svname, status):
        ...
        for i in range(1, self.wait_retries):
            ...
            if state[0]['status'] == status:
                ...
                if self.drain == True and status == 'MAINT':
                    #we should be, now check if the we are currently at 0 sessions
                    if state[0]['scur'] == '0':
                        #we are drained
                        return True
                    else:
                     ...
                     #We only care about draining on bring stuff down, we are in the desired state
                     return True
            else:
                time.sleep(self.wait_interval)
        if in_correct_state == True:
            return True
        else:
